document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadSettings() {
    ...
    const savedMessages = localStorage.getItem("jarvisMessageHistory");
    if (savedMessages) {
      try {
        ...
        localStorage.removeItem("jarvisMessageHistory");
        messageHistory = [];
      }
    }

    window.addEventListener("storage", (event) => {
      if (event.key === "jarvisMessageHistory") {
        loadSettings();
      }
    });
    // Render message history to the chat container
    ...
    async function getAIResponse(query) {
      try {
        ...
        // Update the message in history with the full response
        updateMessageInHistory(messageId, fullResponse);
        speak(fullResponse);
      } catch (error) {
    ...
    function formatAIResponse(text) {
      let formatted = text
        ...
        .replace(
          /^>\s*(.*$)/gm,
          '<blockquote class="neon-quote">$1</blockquote>'
        )
        .replace(
          /```([\s\S]*?)```/g,
    ...
    function addMessage(sender, text, returnId = false) {
      ...
      if (messageHistory.length > config.maxStoredMessages) {
    ...
    // Update message in history (used for streaming AI responses)
    function updateMessageInHistory(id, newText) {
      const messageIndex = messageHistory.findIndex((msg) => msg.id === id);
      if (messageIndex !== -1) {
        messageHistory[messageIndex].text = newText;
        saveMessageHistory();
      }
    }

    // Remove message from chat and history
    function removeMessage(id) {
      const message = document.getElementById(`msg-${id}`);
      if (message) message.remove();

      // Remove from message history
      messageHistory = messageHistory.filter((msg) => msg.id !== id);
      saveMessageHistory();
    }
    ...
    function getLanguageResponse(type) {
      const responses = {
        ...
        chatCleared: {
          ...
          hi: "चैट इतिहास साफ हो गया",
          fr: "Historique de chat effacé",
          de: "Chatverlauf gelöscht",
          hu: "Cset előzmények törölve",
          es: "Historial de chat borrado",
          ar: "تم مسح سجل الدردشة",
          zh: "聊天记录已清除",
          ja: "チャット履歴をクリアしました",
          ru: "История чата очищена",
          pt: "Histórico de chat limpo",
          it: "Cronologia chat cancellata",
        },
        websiteNotFound: {
          en: "Could not find website:",
          hi: "वेबसाइट नहीं मिली:",
          fr: "Impossible de trouver le site web:",
          de: "Website nicht gefunden:",
          hu: "Weboldal nem található:",
          es: "No se pudo encontrar el sitio web:",
          ar: "تعذر العثور على الموقع:",
          zh: "找不到网站:",
          ja: "ウェブサイトが見つかりません:",
          ru: "Не удалось найти веб-сайт:",
          pt: "Não foi possível encontrar o site:",
          it: "Impossibile trovare il sito web:",
        },
        opening: {
          en: "Opening",
          hi: "खोल रहा है",
